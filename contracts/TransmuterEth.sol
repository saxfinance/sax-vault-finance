...
import {VaultWithIndirection} from "./libraries/saxophone/VaultWithIndirection.sol";
...
contract TransmuterEth is Context {
    ...
    address public saToken;
    address public token;

    mapping(address => uint256) public depositedSaTokens;
    ...
    uint256 public totalSupplySaTokens;
    ...
    /// @dev saxophone addresses whitelisted
    ...
    constructor(address _saToken, address _token, address _governance) public {
    ...
    ///@return displays the user's share of the pooled saTokens.
    function dividendsOwing(address account) public view returns (uint256) {
        uint256 newDividendPoints = totalDividendPoints.sub(lastDividendPoints[account]);
        return depositedSaTokens[account].mul(newDividendPoints).div(pointMultiplier);
    }
    ...
    ///@dev Withdraws staked saTokens from the transmuter
    ///
    /// This function reverts if you try to draw more tokens than you deposited
    ///
    ///@param amount the amount of saTokens to unstake
        ...
        updateAccount(msg.sender) 
    {
        ...
        address sender = msg.sender;
        require(depositedSaTokens[sender] >= amount,"Transmuter: unstake amount exceeds deposited amount");
        depositedSaTokens[sender] = depositedSaTokens[sender].sub(amount);
        totalSupplySaTokens = totalSupplySaTokens.sub(amount);
        IERC20Burnable(saToken).safeTransfer(sender, amount);
        emit AlUsdUnstaked(sender, amount);
    }
    ///@dev Deposits saTokens into the transmuter 
    ///
    ///@param amount the amount of saTokens to stake
        ...
        checkIfNewUser()
    {
        ...
        // requires approval of SaToken first
        address sender = msg.sender;
        //require tokens transferred in;
        IERC20Burnable(saToken).safeTransferFrom(sender, address(this), amount);
        totalSupplySaTokens = totalSupplySaTokens.add(amount);
        depositedSaTokens[sender] = depositedSaTokens[sender].add(amount);
        emit AlUsdStaked(sender, amount);
    }
    /// @dev Converts the staked saTokens to the base tokens in amount of the sum of pendingdivs and tokensInBucket
    ///
    /// once the saToken has been converted, it is burned, and the base token becomes realisedTokens which can be recieved using claim()    
    ///
        ...
        updateAccount(msg.sender) 
    {
        ...
        if (pendingz > depositedSaTokens[sender]) {
            diff = pendingz.sub(depositedSaTokens[sender]);

            // remove overflow
            pendingz = depositedSaTokens[sender];
        }

        // decrease satokens
        depositedSaTokens[sender] = depositedSaTokens[sender].sub(pendingz);

        // BURN SATOKENS
        IERC20Burnable(saToken).burn(pendingz);

        // adjust total
        totalSupplySaTokens = totalSupplySaTokens.sub(pendingz);
    ...
    /// @dev Executes transmute() on another account that has had more base tokens allocated to it than saTokens staked.
    ///
        ...
        checkIfNewUser()
    {
        ...
        // check restrictions
        require(
            pendingz > depositedSaTokens[toTransmute],
            "Transmuter: !overflow"
        );
        ...
        uint256 diff = pendingz.sub(depositedSaTokens[toTransmute]);

        // remove overflow
        pendingz = depositedSaTokens[toTransmute];

        // decrease satokens
        depositedSaTokens[toTransmute] = 0;
    ...
    /// @dev Transmutes and unstakes all saTokens
    ///
    /// This function combines the transmute and unstake functions for ease of use
    function exit() public noContractAllowed() {
        transmute();
        uint256 toWithdraw = depositedSaTokens[msg.sender];
        unstake(toWithdraw);
    }

    /// @dev Transmutes and claims all converted base tokens.
    ///
    /// This function combines the transmute and claim functions while leaving your remaining saTokens staked.
    function transmuteAndClaim(bool asEth) public noContractAllowed() {
        transmute();
        claim(asEth);
    }

    /// @dev Transmutes, claims base tokens, and withdraws saTokens.
    ///
    /// This function helps users to exit the transmuter contract completely after converting their saTokens to the base pair.
    ...
    /// @dev Distributes the base token proportionally to all saToken stakers.
    ///
    /// This function is meant to be called by the Saxophone contract for when it is sending yield to the transmuter.
    ...
    /// @dev Allocates the incoming yield proportionally to all saToken stakers.
    ///
    ...
    function increaseAllocations(uint256 amount) internal {
        if(totalSupplySaTokens > 0 && amount > 0) {
            totalDividendPoints = totalDividendPoints.add(
                amount.mul(pointMultiplier).div(totalSupplySaTokens)
            );
    ...
    {
        uint256 _depositedAl = depositedSaTokens[user];
        ...
        uint256 _pendingdivs = _toDistribute.mul(depositedSaTokens[user]).div(totalSupplySaTokens);
        ...
        returns (address[] memory theUserList, uint256[] memory theUserData)
    {
        ...
        for (uint256 x = 0; x < delta; x += 1) {
            ...
            _theUserData[y] = depositedSaTokens[userList[i]];
            _theUserData[y + 1] = dividendsOwing(userList[i]).add(tokensInBucket[userList[i]]).add(_toDistribute.mul(depositedSaTokens[userList[i]]).div(totalSupplySaTokens));
            y += 2;
            i += 1;
        }
    ...
    function migrateFunds(address migrateTo) external onlyGov() {
        ...
        uint256 migratableFunds = totalFunds.sub(totalSupplySaTokens, "not enough funds to service stakes");
    ...
    /// @dev Recover eth sent directly to the Saxophone
    ///
