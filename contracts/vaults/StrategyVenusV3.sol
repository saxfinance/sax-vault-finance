...
interface IWBNB is IERC20 {
    ...
    function unwrapBNB(uint256 wbnbAmount) external;
}
...
interface IMasterPi {
    ...
    function PI() external returns (address);
    function burnAddress() external returns (address);
    function ownerPIReward() external returns (uint256);
    function PIPerBlock() external returns (uint256);
    function startBlock() external returns (uint256);
    function poolInfo(uint256) external returns (
        address want,
        uint256 allocPoint,
        uint256 lastRewardBlock,
        uint256 accPIPerShare,
        address strat
    );
    ...
    function pendingPI(uint256 _pid, address _user) external view returns (uint256);
...
contract StrategyVenus is Initializable, OwnableUpgradeable, ReentrancyGuardUpgradeable, PausableUpgradeable {
     ...
     * Parameters on BSC:
     * _wantAddress（usdt）:
     * _vTokenAddress(xvs vault):
     * _uniRouterAddress(swap router):
     * _venusToWantPath(xvs-usdt):
     */
    function initialize(
    ...
    function setXVSToWantPath(address[] memory newPath) public onlyOwner{
