...
/// @title AlToken
///
/// @dev This is the contract for the Saxophone utillity token usd.
///
...
contract SaToken is AccessControl, ERC20("SA BUSD", "saBUSD") {
  ...
  event SaxophonePaused(address saxophoneAddress, bool isPaused);
  ...
  modifier onlyWhitelisted() {
    require(whiteList[msg.sender], "AlUSD: Saxophone is not whitelisted");
    _;
  }

  /// @dev Mints tokens to a recipient.
  ///
  /// This function reverts if the caller does not have the whiteList role.
  ///
  /// @param _recipient the account to mint tokens to.
  /// @param _amount    the amount of tokens to mint.
  function mint(address _recipient, uint256 _amount) external onlyWhitelisted {
    require(!blacklist[msg.sender], "AlUSD: Saxophone is blacklisted.");
    uint256 _total = _amount.add(hasMinted[msg.sender]);
    require(_total <= ceiling[msg.sender],"AlUSD: Saxophone's ceiling was breached.");
  ...
  /// This function reverts if the caller does not have the admin role.
  function pauseSaxophone(address _toPause, bool _state) external onlySentinel {
    paused[_toPause] = _state;
    SaxophonePaused(_toPause, _state);
  }
  /// This function reverts if the caller does not have the admin role.
  ///
