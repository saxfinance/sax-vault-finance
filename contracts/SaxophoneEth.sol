...
import {CDP} from "./libraries/saxophone/CDP.sol";
import {FixedPointMath} from "./libraries/FixedPointMath.sol";
import {ITransmuter} from "./interfaces/ITransmuter.sol";
import {IMintableERC20} from "./interfaces/IMintableERC20.sol";
import {IChainlink} from "./interfaces/IChainlink.sol";
import {IVaultAdapter} from "./interfaces/IVaultAdapter.sol";
import {Vault} from "./libraries/saxophone/Vault.sol";
import {IWETH9} from "./interfaces/IWETH9.sol";
import {SaEth} from "./SaEth.sol";

contract SaxophoneEth is ReentrancyGuard {
  ...
  constructor(
    ...
    public
  {
    require(_governance != ZERO_ADDRESS, "Saxophone: governance address cannot be 0x0.");
    require(_sentinel != ZERO_ADDRESS, "Saxophone: sentinel address cannot be 0x0.");
  ...
  function setPendingGovernance(address _pendingGovernance) external onlyGov {
    require(_pendingGovernance != ZERO_ADDRESS, "Saxophone: governance address cannot be 0x0.");
  ...
  function setTransmuter(address _transmuter) external onlyGov {
    ...
    require(_transmuter != ZERO_ADDRESS, "Saxophone: transmuter address cannot be 0x0.");
  ...
  function setRewards(address _rewards) external onlyGov {
    ...
    // transfers to the address because of `safeTransfer` checks.
    require(_rewards != ZERO_ADDRESS, "Saxophone: rewards address cannot be 0x0.");
  ...
  function setHarvestFee(uint256 _harvestFee) external onlyGov {
    ...
    require(_harvestFee <= PERCENT_RESOLUTION, "Saxophone: harvest fee above maximum.");
  ...
  function setCollateralizationLimit(uint256 _limit) external onlyGov {
    ...
    require(_limit >= MINIMUM_COLLATERALIZATION_LIMIT, "Saxophone: collateralization limit below minimum.");
    require(_limit <= MAXIMUM_COLLATERALIZATION_LIMIT, "Saxophone: collateralization limit above maximum.");
  ...
  function initialize(IVaultAdapter _adapter) external onlyGov {
    ...
    require(!initialized, "Saxophone: already initialized");

    require(transmuter != ZERO_ADDRESS, "Saxophone: cannot initialize transmuter address to 0x0");
    require(rewards != ZERO_ADDRESS, "Saxophone: cannot initialize rewards address to 0x0");
  ...
  /// @dev Converts ETH/WETH to SaEth
  ///
  ...
  function convert(uint256 _amount, bool asEth) external payable nonReentrant noContractAllowed expectInitialized {
    require(!pauseConvert, "Saxophone: conversions are paused.");
    if (asEth) {
      require(_amount == msg.value, "_amount != msg.value");
      IWETH9(weth).deposit{value: _amount}();
    } else {
    ...
    require(xtoken.hasMinted(address(this)) <= xtoken.ceiling(address(this)),"SaEth: Saxophone's ceiling was breached.");
  ...
  function mint(uint256 _amount) external nonReentrant noContractAllowed expectInitialized {
    ...
    if (_totalCredit < _amount) {
      ...
      _cdp.checkHealth(_ctx, "Saxophone: Loan-to-value ratio breached");
    } else {
  ...
  modifier expectInitialized() {
    require(initialized, "Saxophone: not initialized.");
    _;
  }
  ...
  ///
  modifier onlyGov() {
    require(msg.sender == governance, "Saxophone: only governance.");
    _;
  }
  ...
  ///
  modifier onlyKeeper() {
    require(keepers[msg.sender], "Saxophone: only keepers.");
    _;
  }
  ...
  function _updateActiveVault(IVaultAdapter _adapter) internal {
    require(_adapter != IVaultAdapter(ZERO_ADDRESS), "Saxophone: active vault address cannot be 0x0.");
    require(_adapter.token() == token, "Saxophone: token mismatch.");
  ...
  function _recallFunds(uint256 _vaultId, uint256 _amount) internal returns (uint256, uint256) {
    require(emergencyExit || msg.sender == governance || _vaultId != _vaults.lastIndex(), "Saxophone: not an emergency, not governance, and user does not have permission to recall funds from active vault");
